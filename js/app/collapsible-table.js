import 'jquery';

module.exports = {
    target: '.js-collapsible .js-collapsible-global',
    rowTarget: '.js-collapsible-row',

    sectionTarget: '.js-collapsible tbody',

    init: function() {
        var $container = $(this.target),
            $sections = $(this.sectionTarget),
            $rows = $(this.rowTarget);

        if ($sections.length && $rows.length > 1) {
            this.attachSections($sections, this.rowTarget);
            this.attachGlobal($container, $sections);
        }
    },

    attachGlobal: function($container, $sections) {
        var textShow = 'Show All',
            textHide = 'Collapse',
            $toggler = $(`<a href="#" class="tablesaw-stack-hidden" data-hidden="0">${textHide}</a>`);


        $toggler.on('click', (e) => {
            e.preventDefault();

            var currentHidden = $toggler.data('hidden');
            $toggler.data('hidden', currentHidden === '1' ? '0' : '1');

            // force data-hidden to match global
            $sections.each((i, element) => {
                $(element).find('th a').data('hidden', currentHidden);
            });

            // click
            $sections.each((i, element) => {
                $(element).find('th a').trigger('click');
            });

            var text = currentHidden === '1' ? textHide : textShow;
            $toggler.text(text);
        });

        $container.html($toggler);
    },
    attachSections: function($sections, target) {

        var template = `<a href="#" class="tablesaw-stack-hidden" style="float: right" data-hidden="0">Hide</a>`;

        $sections.each((i, element) => {
            var $toggler = $(template),
                $container = $(element),
                $target = $container.find('th'),
                $children = $container.children(target);

            if ($children.length < 1) {
                return;
            }

            $toggler.on('click', (e) => {
                e.preventDefault();

                if (this.toggle($toggler)) {
                    $children.show();
                    $toggler.text('Hide');
                } else {
                    $children.hide();
                    $toggler.text('Show');
                }
            });

            $target.append($toggler);
        });
    },

    toggle: function($el) {
        // return: bool whether to show or hide target element
        var currentHidden = $el.data('hidden');
        $el.data('hidden', currentHidden === '1' ? '0' : '1');

        return currentHidden === '1' ? true : false;
    }
};
