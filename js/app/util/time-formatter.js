...
function formatTime(iso8601) {
    var time = Date.create(iso8601),
        ...
        relative = time.relative(sugarRelativeAlgo(time));
...
function sugarRelativeAlgo(time) {
    return function (value, unit, ms) {
        ...
        // 2 weeks - 6 months
        } else if (ms > threshold_2week) {
            return '{Month} {d}';

        // 3 days - 2 weeks
        } else if (ms > threshold_3day) {
            return '{Mon} {d}, {h}:{mm} {TT}';

        // 8 hrs - 3 days
        } else if (ms > threshold_8hr) {
            if (time.isToday()) {
                return '{h}:{mm} {TT}';
            } else {
                return '{Dow}, {h}:{mm} {TT}';
        ...
        // 4 hrs - 8 hrs
        } else if (ms > threshold_4hr) {
            return time.hoursAgo() + ' hr  ago';

        // 1 hr - 4 hr
        } else if (ms > threshold_1hr) {
        ...
        // 10 min - 1 hr
        } else if (ms > threshold_10min) {
            return time.minutesAgo() + ' min  ago';

        // 1 min - 10 min
        } else if (ms > threshold_1min) {
